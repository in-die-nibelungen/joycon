...
enum FFB_CTRL
{
	...
	CTRL_STOPALL	= 3,	// Stop All Effectsｭ Issues a stop on every running effect.
...
typedef struct _FFB_EFF_REPORT {
	...
	BYTE		Gain;
	BYTE		TrigerBtn;
	BOOL		Polar; // How to interpret force direction Polar (0-360ｰ) or Cartesian (X,Y)
	union
	{
		BYTE	Direction; // Polar direction: (0x00-0xFF correspond to 0-360ｰ)
...
typedef struct _FFB_EFF_COND {
	BYTE		EffectBlockIndex;
	BOOL		isY;
	LONG 		CenterPointOffset; // CP Offset:  Range -ｭ10000 ｭ- 10000
	LONG 		PosCoeff; // Positive Coefficient: Range -ｭ10000 ｭ- 10000
	LONG 		NegCoeff; // Negative Coefficient: Range -ｭ10000 ｭ- 10000
